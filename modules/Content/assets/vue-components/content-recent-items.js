export default {
    ...
    props: {
        ...
        // Maximum items to fetch per model
        limit: {
            type: Number,
            default: 3
        },
        // How many model sections are expanded initially
        open: {
            type: Number,
            default: 1
        }
    ...
    computed: {
        modelsWithItems() {
            ...
            let models = this.models
                        .filter(model => model.items && model.items.length > 0)
                        .sort((a, b) => {
            ...
            // Set default expanded state for first `open` models if undefined
            models.forEach((m, i) => {
                if (m._visibleItems === undefined) {
                    m._visibleItems = i < this.open;
                }
            });
    ...
    methods: {
        ...
        displayContent(item, model) {
            ...
            Object.keys(item).forEach(key => {
                ...
                const iFields = model.iFields || model.fields || {};

                if (iFields[key]?.type === 'contentItemLink') {

                    if (value._id && value._model) {
                        linkedContent.push({...value, display: iFields[key].opts?.display});
                    }

                    if (Array.isArray(value) && value.length && value[0]._id && value[0]._model) {
                        linkedContent.push({...value[0], display: iFields[key].opts?.display});
                    }
                }
